...
def preorder(t):
    ...
    for b in branches(t):
        res_list += preorder(b)
    return res_list
    # return [label(t)] + [i for b in branches(t) for i in preorder(b)]
...
def has_path(t, word):
    ...
    if len(word) == 1:
        ...
        return (
            False
            if label(t) != word[0]
            else any([has_path(b, word[1:]) for b in branches(t)])
        )
...
def mul_interval(x, y):
    ...
    *Implicit assumption: x and y are two different numbers*
    """
    def data(x):
        return [lower_bound(x), upper_bound(x)]
    tmp = [i * j for i in data(x) for j in data(y)]
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert upper_bound(y) * lower_bound(y) > 0, "The interval used as a divisor spans 0"
    reciprocal_y = interval(1 / upper_bound(y), 1 / lower_bound(y))
    return mul_interval(x, reciprocal_y)


def par1(r1, r2):
    # Refers to r1 and r2 twice.
