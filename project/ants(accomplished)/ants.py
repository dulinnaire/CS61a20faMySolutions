    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if isinstance(self, ContainerAnt):
                judge = self.can_contain(place.ant)
                if judge:
                    self.contain_ant(place.ant)
            elif isinstance(place.ant, ContainerAnt):
                judge = place.ant.can_contain(self)
                if judge:
                    place.ant.contain_ant(self)
            else:
                judge = False
            assert judge, 'Two ants in {0}'.format(place)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while curr_place is not beehive:
            if len(curr_place.bees) > 0 and self.min_range <= transitions_count <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        temp = amount if amount < self.armor else self.armor + self.damage
        for bee in self.place.bees[:]:
            bee.reduce_armor(temp)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        if self.digesting == 0: # Digesting Finished
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt does nothing in a turn but has a large armor value.
    It can protect our home base.
    """
    name = 'Wall'
    food_cost = 4
    implemented = True
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        self.is_first = QueenAnt.uniqueness
        if QueenAnt.uniqueness:
            QueenAnt.uniqueness = False
        # END Problem EC
        ...
        if self.is_first:
            ...
            while curr_place != None:
                if curr_place.ant != None:
                    if not hasattr(curr_place.ant, 'damage_doubled'):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if self.is_first and amount >= self.armor:
            bees_win()
...
class Bee(Insect):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.direction = True # True for going left, False for going right 
        self.scared = False
...
class ContainerAnt(Ant):
    ...
    def contain_ant(self, ant):
        # BEGIN Problem Optional 2
        "*** YOUR CODE HERE ***"
        if ant.place is not None:
            ant.place.ant = None
            ant.place.add_insect(self)
...
def make_scare(action, bee):
    ...
    def scare_action(gamestate):
        if not bee.scared:
            bee.direction = not bee.direction
            bee.scared = True
        action(gamestate)
    return scare_action
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        curr = self.place
        distance = 0
        while curr is not beehive:
            if curr.ant is not None and not isinstance(curr.ant, LaserAnt):
                ret[curr.ant] = distance
