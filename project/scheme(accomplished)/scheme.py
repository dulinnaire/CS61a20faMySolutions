...
def scheme_eval(expr, env, tail=False):
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        # Handling a call expr
        operator = scheme_eval(first, env)
        validate_procedure(operator)
        # Problem 20
        if isinstance(operator, MacroProcedure):
            macro_expr = scheme_apply(operator, rest, env) # apply macro before evaluating the oprands
            while isinstance(macro_expr, Thunk):
                macro_expr = scheme_eval(macro_expr.expr, macro_expr.env)
            return scheme_eval(macro_expr, env) # evaluating the expression returned from the macro
        else:
            oprands = rest.map(lambda expr: scheme_eval(expr, env))
...
def eval_all(expressions, env):
    ...
    # BEGIN PROBLEM 7
    # Problem 19
    def eval_all_tail(expressions, env, tail):
        if scheme_nullp(expressions):
            return None
        if len(expressions) == 1:
            return scheme_eval(expressions.first, env, True)
        scheme_eval(expressions.first, env, False)
        return eval_all_tail(expressions.rest, env, tail)
    return eval_all_tail(expressions, env, False)
    # END PROBLEM 7
...
class Frame(object):
    ...
    def __init__(self, parent):
        ...
        self.bindings = {} # A dictionary
        self.parent = parent # A Frame instance. Global frame's parent is None
    ...
    def make_child_frame(self, formals, vals):
        ...
        for binding in zip(unpack(formals), unpack(vals)):
            new_frame.define(binding[0], binding[1])
        return new_frame
        # END PROBLEM 10
...
def unpack(scheme_list):
    ret = []
    while not scheme_nullp(scheme_list):
        ret.append(scheme_list.first)
        scheme_list = scheme_list.rest
    return ret
...
class LambdaProcedure(Procedure):
    ...
    def __init__(self, formals, body, env):
        ...
        self.body = body
        self.env = env # the frame where it is defined

    def make_call_frame(self, args, env):
        """Make a frame that binds my formal parameters to ARGS, a Scheme list
        of values, for a lexically-scoped call evaluated in environment ENV."""
        # BEGIN PROBLEM 11
        # make a new frame from where it is defined
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        fn = do_lambda_form(Pair(formals, body), env)
        env.define(name, fn)
        return name
        # END PROBLEM 9
    else:
...
def do_quote_form(expressions, env):
    ...
    """
    # expressions has a form of Pair(A, nil), where A is the quoted expression
...
def do_and_form(expressions, env):
    ...
    if len(expressions) == 1:
        ...
        return eval_first\
            if is_true_primitive(eval_first) else False
    else:
...
def do_or_form(expressions, env):
    ...
    if len(expressions) == 1:
        eval_first = scheme_eval(expressions.first, env, True)
        return False\
            if is_false_primitive(eval_first) else eval_first
    else:
        eval_first = scheme_eval(expressions.first, env)
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    for p in unpack(bindings):
        validate_form(p, 2, 2)
        name = unpack(p)[0]
        val = scheme_eval(unpack(p)[1], env)
        scheme_symbolp(name)
...
def do_define_macro(expressions, env):
    ...
    1
    (define-macro (macro-name formals) body)
    """
    # BEGIN Problem 20
    validate_form(expressions, 2) # Checks that expressions is a list of length at least 2
    target = expressions.first
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        # BEGIN PROBLEM 19
        # Use Python iteration to implement tail recursion in Scheme
