...
FIRST_101_DIGITS_OF_PI = 3_1415926535897932384626433832795028841971693993751058209749445923078164062862089986280348253421170679
...
def roll_dice(num_rolls, dice=six_sided):
    ...
    assert num_rolls <= 10, 'num_rolls is up to 10'
    # BEGIN PROBLEM 1
    sum, n = 0, 0
    flag = False
    while n < num_rolls:
...
def swine_align(player_score, opponent_score):
    ...
    if not player_score * opponent_score:
        return False
    while tmp <= min(player_score, opponent_score):
        if player_score % tmp == 0 and opponent_score % tmp == 0:
            divisor = tmp
        tmp += 1
    return divisor >= 10
    # END PROBLEM 4a
...
def pig_pass(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4b
    return 3 > opponent_score - player_score > 0
    # END PROBLEM 4b
...
def who_next(who_now, player_score, opponent_score):
    """Determine which player is the next to have a turn"""
    return who_now if extra_turn(player_score, opponent_score) else other(who_now)
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who:
            ...
            who = who_next(who, score1, score0)
        else:
            score0 += take_turn(strategy0(score0, score1), score1, dice)
            who = who_next(who, score0, score1)
        # BEGIN PROBLEM 6
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    """
    # BEGIN PROBLEM 9
    num_rolls, max_now, average = 1, 1, 0
    while num_rolls <= 10:
        tmp = make_averaged(roll_dice, trials_count)(num_rolls, dice)
        if tmp > average:
            average = tmp
            max_now = num_rolls
        num_rolls += 1
...
def run_experiments():
    ...
    if 1:  # Change to True to test always_roll(8)
        print('always_roll(8) win rate:', average_win_rate(always_roll(8)))

    if 1:  # Change to True to test bacon_strategy
        print('bacon_strategy win rate:', average_win_rate(bacon_strategy))

    if 1:  # Change to True to test extra_turn_strategy
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    print("DEBUG:", extra_turn(new_score, opponent_score) or extra_turn(new_score, opponent_score))
    return 0 if extra_turn(score, opponent_score) or extra_turn(new_score, opponent_score) \
